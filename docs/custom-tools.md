...
工具执行管线 (`createExecutionPipeline`) 提供了可插拔的前置/后置钩子：
...
1. **输入验证** — 调用 `tool.validateInput()`
2. **权限检查** — 调用 `tool.checkPermissions()`
3. **前置钩子** — 执行 preHooks
4. **工具调用** — 调用 `tool.call()`
5. **后置钩子** — 执行 postHooks
...
`orchestrateToolCalls` 函数负责并发/串行编排多个工具调用：
...
for await (const result of orchestrateToolCalls(calls, tools, context)) {
  console.log(result)
}
```
